...
class SBP_model_multExp(nn.Module):
    def __init__(self, zp_calib: bool = False) -> None:
        super().__init__()

        self.encoder1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=9, padding=4),
            nn.MaxPool2d(2), nn.BatchNorm2d(16), nn.ReLU(),
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=7, padding=4),
            nn.MaxPool2d(2), nn.BatchNorm2d(32), nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=5, padding=3),
            nn.MaxPool2d(2), nn.BatchNorm2d(64), nn.ReLU(),
            nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=3),
            nn.MaxPool2d(2), nn.BatchNorm2d(32), nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=16, kernel_size=2, padding=2),
            nn.MaxPool2d(2), nn.BatchNorm2d(16), nn.ReLU()
        )
        ...
        dim_linear = 16*4*4 + 1 if zp_calib else 16*4*4

        self.linear_features = nn.Sequential(
            nn.Linear(dim_linear, 200),
            nn.Dropout(0.02),
            nn.ReLU(),
            nn.Linear(200, 50),
            nn.Dropout(0.02),
        ...
        self.ll_alpha = nn.Linear(10, 5)
        self.ll_sigma = nn.Linear(10, 5)
        self.ll_mean = nn.Linear(10, 5)
        
    def forward(self, img: torch.Tensor, 
                zp: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        logalpha = logalpha - torch.logsumexp(logalpha, 1, keepdim=True)
...
class SBP_model_singExp(nn.Module):
    ...
    def forward(self, img: torch.Tensor, 
                ...
                mode: str = 'train') -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
